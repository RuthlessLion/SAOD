...
void init::siftDown(int *numbers, int root, int bottom) {
    int maxChild;
    int done = 0;
    record* buf;
...
void init::heapSort(int *numbers, int array_size) {
    record* buf;
...
void init::sortDBname(){
    readDB();
    record *buf;
    sort(DBSize);
    printDB();
}
int init::curr(int a,int b){
    for (int i = 0; i<30;i++){
